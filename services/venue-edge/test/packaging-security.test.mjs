...
test("runtime pins use immutable HTTPS URLs and mandatory SHA-256 values", async () => {
  const pins = JSON.parse(await readFile(join(serviceRoot, "packaging", "pins.json"), "utf8"))
...
test("release signing fails closed and verifies Authenticode", async () => {
  const script = await readFile(join(serviceRoot, "packaging", "sign.ps1"), "utf8")
...
test("Windows service uses a restricted account and data ACL", async () => {
  const [xml, acl] = await Promise.all([
    readFile(join(serviceRoot, "packaging", "winsw", "PlayTTVenueEdge.xml"), "utf8"),
    readFile(join(serviceRoot, "packaging", "acl.ps1"), "utf8"),
  ])
  assert.match(xml, /<user>LocalService<\/user>/)
  assert.match(acl, /LOCAL SERVICE:\(OI\)\(CI\)M/)
})
...
test("SBOM contains pinned component hashes", async () => {
  ...
  assert.equal(document.packages.filter((entry) => entry.checksums?.[0]?.algorithm === "SHA256").length, 3)
})

test("AC sleep and unverified policies are surfaced as risks", () => {
  assert.equal(assessWindowsPowerSnapshot({ onBattery: false, acSleepSeconds: 900 }).hostSleepRisk, true)
  assert.equal(assessWindowsPowerSnapshot({ onBattery: false, acSleepSeconds: 0, acHibernateSeconds: 0 }).hostSleepRisk, false)
})
