...
test("venue-edge fleet server module and APIs exist", () => {
  ...
  assert.match(fleet, /heartbeatMetrics/)
  assert.match(fleet, /sourceHealthRows/)
  assert.match(fleet, /isNull\(venueEdgeInstallations\.retiredAt\)/)
  assert.match(fleet, /parseHeartbeatMetrics/)
  assert.match(fleet, /pinnedVersion/)
  assert.match(fleet, /lastUpdateErrorCode/)
  ...
  assert.match(operatorActions, /retiredAt/)
...
test("fleet UI surfaces do not embed device or NVR secrets", () => {
  ...
  assert.match(detail, /update-actions/)
  assert.match(detail, /Download diagnostics/)
