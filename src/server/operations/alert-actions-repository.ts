...
import type { AlertDispatchAuditEntry } from "./alert-dispatch-types.ts"

export async function listRecentlyAcknowledgedAlertIds(
  tenantId: string,
  since: Date,
): Promise<Set<string>> {
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(auditLogs.action, OPERATIONAL_ALERT_ACKNOWLEDGED_ACTION),
...
export async function listRecentAlertActionAudit(
  tenantId: string,
  limit = 20,
    ...
    .where(
      and(
        ...
        inArray(auditLogs.action, [
          OPERATIONAL_ALERT_DISPATCH_ACTION,
          OPERATIONAL_ALERT_DISPATCH_FAILED_ACTION,
          OPERATIONAL_ALERT_ACKNOWLEDGED_ACTION,
        ]),
      ),
    )
  ...
  return rows
    ...
    .map((row) => {
      ...
      return {
        ...
        success:
          row.action === OPERATIONAL_ALERT_DISPATCH_ACTION ||
          row.action === OPERATIONAL_ALERT_ACKNOWLEDGED_ACTION,
        channel:
