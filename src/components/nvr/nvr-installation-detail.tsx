...
export function NvrInstallationDetail({ installation: initialInstallation, canManage }: { installation: VenueEdgeInstallationDetailView; canManage: boolean }) {
  ...
  async function postUpdateAction(action: "change_channel" | "pin_version" | "retry_update", extra?: Record<string, string | null>) {
    ...
    <div className="space-y-5">
      ...
      <details id="technician" className="rounded-2xl bg-card ring-1 ring-border">
        ...
        <div className="space-y-6 border-t p-5">
          ...
          <section><h3 className="font-semibold">Software updates</h3><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-muted-foreground">Current version</dt><dd className="font-medium">{installation.currentAgentVersion}</dd></div><div><dt className="text-muted-foreground">Desired version</dt><dd>{installation.desiredAgentVersion ?? "—"}</dd></div><div><dt className="text-muted-foreground">Update status</dt><dd>{installation.updateStatus}</dd></div><div><dt className="text-muted-foreground">Channel</dt><dd>{installation.updateChannel}{installation.pinnedVersion ? ` · pinned ${installation.pinnedVersion}` : ""}</dd></div><div><dt className="text-muted-foreground">Last update</dt><dd>{formatDate(installation.lastUpdateAt)}</dd></div><div><dt className="text-muted-foreground">Last update error</dt><dd>{installation.lastUpdateErrorCode ?? "—"}</dd></div></dl>{canManage ? <div className="mt-4 flex flex-wrap gap-2"><Button variant="outline" disabled={isPending || !reasonReady} onClick={() => { setMessage(null); startTransition(async () => { try { await postUpdateAction("retry_update"); setMessage("Update retry requested."); await refreshDetail() } catch (error) { setMessage(error instanceof Error ? error.message : "Update retry failed.") } }) }}>Retry update</Button><Button variant="outline" disabled={isPending || !reasonReady} onClick={() => { setMessage(null); startTransition(async () => { try { await postUpdateAction("change_channel", { channel: "stable" }); setMessage("Channel set to stable."); await refreshDetail() } catch (error) { setMessage(error instanceof Error ? error.message : "Channel change failed.") } }) }}>Use stable channel</Button><Button variant="outline" disabled={isPending} onClick={() => { setMessage(null); startTransition(async () => { try { await downloadDiagnostics(); setMessage("Diagnostics bundle downloaded.") } catch (error) { setMessage(error instanceof Error ? error.message : "Diagnostics failed.") } }) }}>Download diagnostics</Button></div> : null}</section>
