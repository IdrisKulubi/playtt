...
export function SiteFooterMotion({ children }: { children: ReactNode }) {
  ...
  useGSAP(() => {
    ...
    if (prefersReducedMotion) {
      gsap.set([word, ...groups, base], { clearProps: "all", opacity: 1 });
      return;
    }

    const timeline = gsap.timeline({ scrollTrigger: { trigger: scope, start: "top 82%", once: true } });
    timeline.fromTo(word, { yPercent: 120, opacity: 0 }, { yPercent: 0, opacity: 1, duration: 0.85, ease: MARKETING_EASE_OUT });
    timeline.fromTo(groups, { y: 26, opacity: 0 }, { y: 0, opacity: 1, duration: 0.55, stagger: 0.08, ease: MARKETING_EASE_OUT }, 0.28);
    timeline.fromTo(base, { opacity: 0 }, { opacity: 1, duration: 0.45 }, 0.7);
