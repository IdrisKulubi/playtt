...
| P3-02 | Assignments and configuration | Time-aware device-to-resource roles, configuration versions, and per-resource capability checks support reassignment without reflashing. | **Implemented, acceptance pending** — capability policy, half-open overlap constraints, monotonic config acknowledgement |
| P3-03 | Heartbeat and fleet health | HTTPS heartbeat records latest health and sampled history; configurable offline detection is resource-isolated. | **Implemented, acceptance pending** — serialized sampling, monotonic/future-safe health, scheduled retention |
| P3-04 | Device commands and acknowledgements | Expiring commands, retries, acknowledgement state, correlation, and audit are implemented behind `DeviceCommandBus`. | **Implemented, acceptance pending** — max attempts, redelivery, expiry/failure reconciliation, serialized ACK; session/outbox command producer remains |
