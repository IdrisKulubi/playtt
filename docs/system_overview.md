...
* **Flow:**
  ...
  4. VenueEdge extracts the configured clip window from the local rolling buffer (or validated NVR fallback) and uploads directly to private Cloudflare R2 via an exact-object PUT grant.
...
* `replays`: Links ready replay projections to bookings and users; private clips reference `media_assets` in R2 (not public S3 URLs).